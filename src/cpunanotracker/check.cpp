// Local integration check using real ONNX models and CPU inference, no test framework.
#include <gst/gst.h>
...
class Pipeline {
public:
    explicit Pipeline(const char* models)
    {
        GError* error = nullptr;
        pipeline = gst_parse_launch("appsrc name=input format=time is-live=true ! "
            "cpunanotrack name=tracker ! appsink name=output sync=false async=false", &error);
        if (error) {
        ...
        g_object_set(tracker, "models-dir", models, nullptr);
...
{
    gst_init(&argc, &argv);
    require(argc >= 2, "Usage: cpunanotracker-check MODELS_DIR");
    {
        Pipeline idle("");
    ...
    }
    {
        ...
        g_object_set(p.tracker, "models-dir", "/must-not-change", nullptr);
        gchar* dir = nullptr;
        g_object_get(p.tracker, "models-dir", &dir, nullptr);
        const bool unchanged = std::string(dir) == argv[1];
        g_free(dir);
        require(unchanged, "Stopped-only properties changed while PLAYING");
        p.frame(0);
        Pipeline other(argv[1]);
        g_object_set(other.tracker, "roi", "1,2,32,32", "enabled", TRUE, nullptr);
        other.play(); other.frame(1); p.frame(0); other.frame(0);
        // Small ROIs still initialize and resize entirely on the CPU.
    ...
    {
        Pipeline p("/nonexistent-nanotracker-models");
        g_object_set(p.tracker, "roi", "0,0,32,32", "enabled", TRUE, nullptr);
        p.expect_start_error();
    }
    std::cout << "PASS: CPU tracking, passthrough, controls, resets, independent instances, and errors\n";
    return 0;
